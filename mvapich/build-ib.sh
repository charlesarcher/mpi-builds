...
COMPILER=$1
PWD=$(pwd)
MPICH2DIR=${PWD}/mvapich2-2.1-3/
MPICH2DIR=$(pwd)/mvapich2-2.1rc2/
...
#        EXTRA_OPT="${EXTRA_OPT} -fvisibility=hidden -fvisibility-default=${PWD}/pubsym.txt"
#        EXTRA_DEBUG="${EXTRA_DEBUG} -fvisibility=hidden -fvisibility-default=${PWD}/pubsym.txt"
        ;;
    gnu )
...
#        EXTRA_OPT="${EXTRA_OPT} -fvisibility=hidden"
#        EXTRA_DEBUG="${EXTRA_DEBUG} -fvisibility=hidden"
#        EXTRA_LD_OPT="${EXTRA_LD_OPT} -Wl,--version-script=${PWD}/pubsym.ver"
#        EXTRA_LD_DEBUG="${EXTRA_LD_DEBUG} -Wl,--version-script=${PWD}/pubsym.ver"
        ;;
    clang )
        if [ -e /opt/rh/devtoolset-3/enable ]; then
            . /opt/rh/devtoolset-3/enable
